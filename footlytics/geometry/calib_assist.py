...
def write_assist(frame: np.ndarray, out_stem: str | Path, person_boxes: np.ndarray | None = None,
                 anchor_frame: int = 0) -> tuple[Path, Path]:
    """Write an annotated image and a JSON with candidates plus an empty calibration template.

    The template uses the upstream landmark names (`Pitch.landmarks()`); fill `image_points`
    with `{landmark_name: [px, py]}` for the candidates you recognise, then build the
    calibration with `footlytics calibrate` (Calibration.from_correspondences).
    """
    from .pitch import DEFAULT_PITCH

    res = detect_pitch_intersections(frame, person_boxes)
    out_stem = Path(out_stem)
    out_stem.parent.mkdir(parents=True, exist_ok=True)
    img_path = out_stem.with_suffix(".jpg")
    ...
    payload = {
        ...
        "lines": res["lines"],
        "landmark_names": sorted(DEFAULT_PITCH.landmarks()),
        "template": {"image_points": {}, "anchor_frame": anchor_frame, "image_size": [w, h]},
        "how_to": "Open the .jpg, pick candidates you recognise, copy their x/y into template.image_points "
                  "keyed by landmark name (e.g. halfway_T, corner_RT, pen_area_RB_front), then run "
                  "`footlytics calibrate ASSIST.json --out calib/venue.json`.",
    }
